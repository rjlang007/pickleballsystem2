...
if ($method === 'GET') {
    ...
    if (isset($_GET['menu'])) {
        ...
        $sql = "SELECT id, name, description, price, category, image, is_available, sort_order
...
if ($method === 'POST') {
    ...
    if (in_array($action, ['save_item', 'delete_item'], true) && !$admin) {
    ...
    if ($action === 'save_item') {
        ...
        $category    = trim((string)($body['category'] ?? 'Main')) ?: 'Main';
        ...
        if ($id > 0) {
            $db->prepare(
                "UPDATE falcon.food_items
                    SET name=?, description=?, price=?, category=?, image=?, is_available=?, sort_order=?, updated_at=NOW()
                  WHERE id=?"
            )->execute([$name, $description, $price, $category, $image, $isAvailable, $sortOrder, $id]);
        } else {
            $stmt = $db->prepare(
                "INSERT INTO falcon.food_items
                    (name, description, price, category, image, is_available, sort_order, created_by, created_at, updated_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
                 RETURNING id"
            );
            $stmt->execute([$name, $description, $price, $category, $image, $isAvailable, $sortOrder, $uid]);
