...
function findBestAvailableCourt(PDO $db): ?array {
    $stmt = $db->query("
        SELECT c.id, c.name, c.short_code, c.color, c.court_type,
               c.game_duration, c.warmup_mins, c.pass_hours,
               c.is_active, c.is_maintenance, c.credit_cost, c.max_queue,
               COALESCE(gq.queued, 0) AS queue_count,
               CASE WHEN gs.id IS NULL THEN 0 ELSE 1 END AS has_active_game
        FROM falcon.courts c
        LEFT JOIN (
            SELECT court_id, COUNT(*) AS queued
            FROM falcon.game_queue
            WHERE session_id IS NULL
            GROUP BY court_id
        ) gq ON gq.court_id = c.id
        LEFT JOIN falcon.game_sessions gs
            ON gs.court_id = c.id AND gs.status = 'active'
        WHERE c.is_active = TRUE
          AND c.is_maintenance = FALSE
          AND COALESCE(gq.queued, 0) < c.max_queue
        ORDER BY has_active_game ASC, COALESCE(gq.queued, 0) ASC, c.sort_order ASC, c.id ASC
        LIMIT 1
    ");
