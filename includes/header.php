...
<nav class="navbar">
    ...
    <?php if ($user): ?>
        ...
        <?php if (isSuperAdmin() && !isImpersonating()): ?>
            ...
            <div class="nav-group" id="ng-sa-manage">
                <button class="nav-group-btn" onclick="toggleNavGroup('ng-sa-manage')">🏆 Manage <span class="arrow">▾</span></button>
                <div class="nav-dropdown">
                    ...
                    <a href="<?= APP_URL ?>/admin/season_management.php">🗓️ Season Management</a>
                    <div class="sep"></div>
                    <div class="section-label">Leaderboard</div>
                    <a href="<?= APP_URL ?>/admin/leaderboard_admin.php">📊 Leaderboard Admin</a>
                    <a href="<?= APP_URL ?>/admin/leaderboard_settings.php">⚙️ Leaderboard Settings</a>
                    <a href="<?= APP_URL ?>/admin/achievements_admin.php">🥇 Achievements</a>
                    <div class="sep"></div>
                    <div class="section-label">Site &amp; Announcements</div>
            ...
            <div class="nav-group" id="ng-sa-acct">
                <button class="nav-group-btn nav-acct-btn" onclick="toggleNavGroup('ng-sa-acct')">
        ...
        <?php elseif (isAdmin()): ?>
            ...
            <div class="nav-group" id="ng-tournaments">
                <button class="nav-group-btn" onclick="toggleNavGroup('ng-tournaments')">🏆 Tournaments <span class="arrow">▾</span></button>
                <div class="nav-dropdown">
                    <a href="<?= APP_URL ?>/admin/tournament_admin.php">🏆 Create / Manage Tournaments</a>
                    <a href="<?= APP_URL ?>/staff/open_play_control.php">🎲 Open Play Control</a>
                    <a href="<?= APP_URL ?>/admin/tournament_bracket_editor.php">🗂️ Bracket Editor</a>
        ...
        <?php elseif (isRefereeOnly()): /* Referee desktop nav */ ?>
            ...
            <a href="<?= APP_URL ?>/public/tournaments.php">🏆 Tournaments</a>
            <a href="<?= APP_URL ?>/public/open_play.php">🎲 Open Play</a>
            <a href="<?= APP_URL ?>/public/leaderboard.php">📊 Standings</a>
...
<?php if ($user):
    // ── Club-wide announcements banner — visible to every logged-in
    // account (player/staff/referee/admin/super_admin), audience-filtered.
    // Posted via admin/announcements.php by admin or super_admin.
    try {
        $_annStmt = getDB()->prepare(
            "SELECT id, title, body, is_pinned
               FROM falcon.announcements
        ...
        $_annStmt->execute([':role' => $_SESSION['role'] ?? 'player']);
        $_announcements = $_annStmt->fetchAll();
    } catch (Throwable $e) {
        $_announcements = [];
    }
?>
<?php if (!empty($_announcements)): ?>
<div class="fcp-announce-wrap">
    <?php foreach ($_announcements as $_ann): ?>
    <div class="fcp-announce-bar<?= $_ann['is_pinned'] ? ' fcp-announce-pinned' : '' ?>" data-ann-id="<?= (int)$_ann['id'] ?>">
        <span class="fcp-announce-icon"><?= $_ann['is_pinned'] ? '📌' : '📢' ?></span>
        <span class="fcp-announce-text"><strong><?= clean($_ann['title']) ?>:</strong> <?= clean($_ann['body']) ?></span>
        <button type="button" class="fcp-announce-close" aria-label="Dismiss announcement" onclick="this.closest('.fcp-announce-bar').remove()">✕</button>
    </div>
    <?php endforeach; ?>
</div>
<style nonce="<?= getCspNonce() ?>">
    .fcp-announce-wrap{max-width:1200px;margin:8px auto 0;padding:0 16px;}
    .fcp-announce-bar{display:flex;align-items:center;gap:10px;background:#eef6ff;border:1px solid #cfe4fb;color:#1c4d80;border-radius:8px;padding:9px 14px;margin-bottom:8px;font-size:13.5px;}
    .fcp-announce-pinned{background:#fff6e5;border-color:#f3d9a1;color:#7a4f00;}
    .fcp-announce-text{flex:1;}
    .fcp-announce-close{background:none;border:none;cursor:pointer;color:inherit;opacity:.6;font-size:14px;}
    .fcp-announce-close:hover{opacity:1;}
</style>
<?php endif; endif; ?>
...
<?php if ($user): ?>
    ...
    <?php if (isSuperAdmin() && !isImpersonating()): ?>
        ...
        <div class="mob-section-label">Site &amp; Announcements</div>
        <a href="<?= APP_URL ?>/admin/announcements.php">📢 Announcements</a>
        <a href="<?= APP_URL ?>/admin/content_manager.php">✏️ Content Manager</a>
        <a href="<?= APP_URL ?>/admin/activity_manager.php">🎯 Activity Center</a>
        <a href="<?= APP_URL ?>/admin/court_mode.php">🏓 Court Mode</a>
    ...
    <?php elseif (isAdmin()): ?>
        ...
        <div class="nav-group" id="ng-tournaments-m">
            <button class="nav-group-btn" onclick="toggleNavGroup('ng-tournaments-m')">🏆 Tournaments <span class="arrow">▾</span></button>
            <div class="nav-dropdown">
                <a href="<?= APP_URL ?>/admin/tournament_admin.php">🏆 Create / Manage Tournaments</a>
                    <a href="<?= APP_URL ?>/staff/open_play_control.php">🎲 Open Play Control</a>
                <a href="<?= APP_URL ?>/admin/tournament_bracket_editor.php">🗂️ Bracket Editor</a>
