...
if ($isAdmin && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_location') {
    ...
    if (!checkRateLimit('map_save_' . (int)($_SESSION['user_id'] ?? 0), 20, 300)) {
        http_response_code(429);
        echo json_encode(['ok' => false, 'error' => 'Too many requests. Please wait.']); exit;
    }
    ...
    try {
        ...
        // Don't echo $e->getMessage() to the client — it can leak schema/
        // column details. Log server-side, return a generic message.
        error_log('[map save] ' . $e->getMessage());
